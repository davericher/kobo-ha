...
connectWebSocket();
seedStatesFromRest().catch(e => console.error(`Something went wrong seeding states: ${e?.message || ''}`));
...
async function buildLandscapeCanvas() {
    ...
    for (let i = 0; i < locRowCount; i++) {
        ...
        const verticalNudge = 2; // move text up a bit
...
// Value (auto-sized)
    ...
    // ---------- Date box (bottom half of bottom-right) ----------
// Date box: vertical DOW at left, big day number to its right
...
// Narrow vertical stripe for DEC / THU
    const dowAreaW = Math.min(dateW * 0.3, 80);
    const dowArea = [dateLeft, dateTop, dateLeft + dowAreaW, dateBottom];

// Day number lives between the DOW strip and the same vertical
// column boundary used by the Location table (locMidX)
    ...
    // Draw DOW and month vertically stacked in the left stripe
    {
    ...
    // Big day number + suffix in the remaining area on the left side
    ...
    {
        // Choose a font that fits, but we’ll anchor from the bottom
        const {font} = fitTextInBox(ctx, dayStr, dayBox, 140, 60);
        ctx.font = font;

        // Re-measure with the chosen font
        const {w, h} = textSize(ctx, dayStr);

        // Centre the day number a bit left (around 40% of the box width)
        const boxCenterX = (dayBox[0] + dayBox[2]) / 2;
        const textCenterX =
            dayBox[0] + (boxCenterX - dayBox[0]) * 0.8; // 0.8 * half-width => 40% of total
        const dayX = textCenterX - w / 2 - 10;

        // Anchor from the bottom so we never cross the lower border,
        // and lift it up by 10px.
        const bottomMargin = 4; // space from bottom border
        const lift = 10;        // move up 10px
        const dayY = dayBox[3] - h - bottomMargin - lift;

        ctx.fillText(dayStr, dayX, dayY);

        // Small suffix in the top-right of the number, clamped into the box
...
//  - separates labels/values in the Location table
//  - continues straight down through the Date box
    ctx.beginPath();
    ...
    //  - only between rows (no extra header line above "Dave")
